...
function dia_alert(tit, msg, cb) {
    if (!!!tit || !!!msg) {
        return false
    } else {
        if ($("body").find("#maskA").length == 0) {
            $("body").append('<div class="mask" id="maskA">' + '<div class="alert_green">' + '<div class="alert_con">' + '<div class="alert_tit"><span>' + tit + '</span><div class="close"><span>×</span></div></div>' + '<div class="alert_body"><br><span class="t_gray">' + msg + '</span><br>' + '<br>' + '<div style="text-align: center">' + '<a href="javascript:;" class="btn_green btn_bg45" >确定</a>' + '</div>' + '<br>' + '</div>' + '</div>' + '</div>' + '</div>')
        } else {
            $("#maskA").find(".alert_tit>span").html(tit);
            $("#maskA").find(".alert_body").html(msg)
        }
        $("#maskA").show();
        $("#maskA").find(".btn_green").on("click", function () {
            $("#maskA").hide();
            if (!!cb) {
                cb()
            }
...
function dia_conform(tit, msg, cb) {
    if (!!!tit || !!!msg) {
        return false
    } else {
        if ($("body").find("#maskC").length == 0) {
            $("body").append('<div class="mask" id="maskC">' + '<div class="alert_green">' + '<div class="alert_con">' + '<div class="alert_tit"><span>' + tit + '</span><div class="close"><span>×</span></div></div>' + '<div class="alert_body"><br><span class="t_gray">' + msg + '</span><br>' + '<input type="text" placeholder="" class="inputgray" />' + '<br>' + '<div style="text-align: center">' + '<a href="javascript:;" class="btn_gray btn_bg45" >取消</a><a href="javascript:;" class="btn_green btn_bg45" >确定</a>' + '</div>' + '<br>' + '</div>' + '</div>' + '</div>' + '</div>')
        } else {
            $("#maskC").find(".alert_tit>span").html(tit);
            $("#maskC").find(".alert_body").html(msg)
        }
        $("#maskC").show();
        $("#maskC").find(".btn_green").on("click", function () {
            $("#maskC").hide();
            if (!!cb) {
                cb()
            }
        });
        $("#maskC").find(".btn_gray").on("click", function () {
            $("#maskC").hide()
        })
    }
    return false
...
function addImg(obj, len) {
    ...
    function compress(a) {
        ...
        var l = can.toDataURL("image/jpeg", 0.1);
...
function imglistInit(i) {
    $(".imglist3 .img").height($(".imglist3 .aimg").width());
