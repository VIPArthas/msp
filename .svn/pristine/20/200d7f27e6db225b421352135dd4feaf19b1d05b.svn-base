...
<div class="main">
  <div class="search_box">
    <form action="<%=request.getContextPath()%>/admin/adminUserPay/web/toIndex.htm" method="post" id="search_form">
      <table cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td>
            <div class="chaxun" style="background-image:none;"><input style="text-indent:0px;"   type="button" class="btn"  onClick="javascript:location.href='<%=request.getContextPath()%>/admin/adminUserPay/web/toIndex.htm';" value="清空"></div>
          </td>
        </tr>
        </tr>
      </table>
  ...
  <div class="lb_bd">
    <table width="100%"  border="0" class="table" align="center" cellpadding="0"
      ...
      <tr align="center">
        ...
        <th width="10%" align="center" > 交易的金额
        ...
        <th width="10%" align="left" > 备注
          <a href="javascript:void(0);" onclick="sortSubmit('up.pay_note desc')">▼</a>
          <a href="javascript:void(0);" onclick="sortSubmit('up.pay_note asc')">▲</a>
        </th>
      </tr>

      <c:choose>
        <c:when test="${empty userPayList[0]}">
          <tr>
            <td colspan="11" align="center">没有符合条件的记录</td>
          </tr>
        </c:when>
        <c:otherwise>
          <c:forEach items="${userPayList[0]}" var="userPay" varStatus="status">
            <tr align="center">
              <td align="center">${userPay.pay_num}</td>
              <td align="center"><fmt:formatDate value="${userPay.pay_time}" pattern="yyyy-MM-dd " /> </td>
              <td align="center">${userPay.pay_school_money}</td>
              <td align="center">${userPay.phone}</td>
              <td align="center">${userPay.real_name}</td>
              <td align="center">${userPay.school_name}</td>
              <td align="center">${userPay.order_info}</td>
              <td>
                <c:choose>
                  <c:when test="${userPay.pay_status == 0}">
                    系统托管
                  </c:when>
                  <c:when test="${userPay.pay_status == 1}">
                    成功
                  </c:when>
                  <c:when test="${userPay.pay_status == 2}">
                    失败
                  </c:when>
                </c:choose>
              </td>
              <td align="center">${userPay.pay_note}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
  ...
  <page:page currentpage="${userPayList[1]}" rscount="${userPayList[2]}"
             className="adminPage" pagesize="5" action="/admin/adminUserPay/web/toIndex.htm"/>
</div>
<!--弹出层-->
