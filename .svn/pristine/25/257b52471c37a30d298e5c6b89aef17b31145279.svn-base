...
function time(tim){
 ...
 function suipainum(first) {
	 ...
	 var type = $('.active_type').attr('data-type');
        ...
        $.ajax({
            ...
            success: function (result) {
            		if(result.obj){
                        if(firs){
                      	    localStorage.setItem('spobj',JSON.stringify(result.obj));  
                        	document.querySelector('.waterfall').innerHTML = '';
                        }else if(result.obj.length>0){
                        	console.log(result.obj);
                        	var puboobj = JSON.parse(localStorage.getItem('spobj'));
                        	for(var i=0;i<result.obj.length;i++){
                        		puboobj.push(result.obj[i]);
                        	}
                        	localStorage.setItem('spobj',JSON.stringify(puboobj));
                        }
            	        pubu(result.obj);
            			...
            			if(firs){
            		        ...
            		        $('.nothing').css({"color": "#999","text-align": "center","font-size": "3vw","margin": "3vw"});
            			}
 ...
 function pubu(obj){
	    ...
	    if(!!localStorage.getItem('sptop')){
	    	document.querySelector('body').scrollTop = localStorage.getItem('sptop');
...
function daiban(){
	...
	$.ajax({
        ...
        success: function (result) {
        	if(result.success){       		
        		if(result.obj){
        			localStorage.setItem('classna',JSON.stringify(result));
        			showdaiban(result);
        		}else{
...
//		});
	    if(!!localStorage.getItem('classindex')){
		    $('.tittle_list').eq(localStorage.getItem('classindex')).addClass('on');
	...
	if(result.attributes!=null){
		...
		for(var j = 0;j < result.attributes.muList.length;j++){
	        ...
	        stulist.innerHTML = (result.attributes.muList[j].gender==1?'<span class="tittle_main_ico_t icon iconfont icon-iconwohover color_1"></span>':'<span class="tittle_main_ico_t icon iconfont icon-iconwohover color_2"></span>')
				                +'<div class="bottom-line"><span class="tittle_main_name">' + result.attributes.muList[j].name + '</span>'
				                +'<a href="tel:'+result.attributes.muList[j].mobile+'" class="tittle_main_ico_f icon iconfont icon-boda"></a></div>';
	        stuul.appendChild(stulist);
	    	
	    }
		
		if(!!localStorage.getItem('top')){
			$('#student_ul').css('top',localStorage.getItem('top'));
			localStorage.removeItem('top');
		}else{
			$('#student_ul').css('top','0');

		}
		$('.tittle_main').bind('click', function (event) {
			event.stopPropagation();
			localStorage.setItem('classindex',$('.on').index());
			localStorage.setItem('top',$('#student_ul').css('top'));
		...
		var stuul = document.querySelector('#student_ul');
		stuul.innerHTML = '<img src="../../img/empty02.png" class="empty_stu" alt="">';
	        ...
	        success: function (result) {
	        	...
	        	if(result.success){
	        		if(result.attributes!=null){	
	        			...
	        			var classna =  JSON.parse(localStorage.getItem('classna'));
	        			classna.attributes = result.attributes;
	        			localStorage.setItem('classna',JSON.stringify(classna));
