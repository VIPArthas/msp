...
<div class="main">
  <div class="search_box">
    <form action="<%=request.getContextPath()%>/admin/adminUserCash/web/toIndex.htm" method="post" id="search_form">
      <table cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td width="100" align="right"><span>申请日期查询:</span></td>
          <td>
          ...
          <td>
            <div class="chaxun" style="background-image:none;"><input style="text-indent:0px;"   type="button" class="btn"  onClick="javascript:location.href='<%=request.getContextPath()%>/admin/adminUserCash/web/toIndex.htm';" value="清空"></div>
          </td>
        </tr>
        </tr>
      </table>
    ...
    <table width="100%"  border="0" class="table" align="center" cellpadding="0"
      ...
      <tr align="center">
        <th width="7%" align="left" > 交易流水号
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_num desc')">▼</a>
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_num asc')">▲</a>
        </th>
        <th width="10%" align="left" > 提现申请时间
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_time desc')">▼</a>
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_time asc')">▲</a>
        </th>
        <th width="10%" align="center" > 提现金额
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_money desc')">▼</a>
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_money asc')">▲</a>
        </th>
        <th width="10%" align="center" > 银行
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_bank desc')">▼</a>
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_bank asc')">▲</a>
        </th>
        <th width="10%" align="center" > 银行卡号
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_card desc')">▼</a>
          <a href="javascript:void(0);" onclick="sortSubmit('uc.cash_card asc')">▲</a>
        </th>
        ...
        <th width="10%" align="left" > 提现申请时ip
          <a href="javascript:void(0);" onclick="sortSubmit('uc.operate_ip desc')">▼</a>
          <a href="javascript:void(0);" onclick="sortSubmit('uc.operate_ip. asc')">▲</a>
        </th>
      </tr>

      <c:choose>
        <c:when test="${empty userCashList[0]}">
          <tr>
            <td colspan="11" align="center">没有符合条件的记录</td>
          </tr>
        </c:when>
        <c:otherwise>
          <c:forEach items="${userCashList[0]}" var="userCash" varStatus="status">
            <tr align="center">
              <td align="center">${userCash.cash_num}</td>
              <td align="center"><fmt:formatDate value="${userCash.cash_time}" pattern="yyyy-MM-dd " /> </td>
              <td align="center">${userCash.cash_money}</td>
              <td align="center">${userCash.cash_bank}</td>
              <td align="center">${userCash.cash_card}</td>
              <td align="center">${userCash.phone}</td>
              <td align="center">${userCash.real_name}</td>
              <td align="center">${userCash.operate_ip}</td>
            </tr>
          </c:forEach>
        </c:otherwise>
      </c:choose>
  ...
  <page:page currentpage="${userCashList[1]}" rscount="${userCashList[2]}"
             className="adminPage" pagesize="5" action="/admin/adminUserCash/web/toIndex.htm"/>
</div>
<!--弹出层-->
